...
export default function HomeImprovementHub() {
  return (
    <div className="flex flex-col min-h-screen bg-[#f5f7f8] dark:bg-[#101922] font-[Space_Grotesk,system-ui,sans-serif] text-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-14 z-20 bg-[#f5f7f8]/80 dark:bg-[#101922]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center px-4 py-3 gap-3 max-w-3xl mx-auto w-full">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold">Home Improvement</h1>
            <p className="text-xs text-slate-400">Tools, materials, plumbing & electrical</p>
          </div>
        </div>
      </header>
      <main className="pb-24 md:pb-6">
        <div className="max-w-3xl mx-auto w-full px-4 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <SectionCard title="Power & Hand Tools" icon="hardware" iconClass="text-orange-500">
            <FeedPreview widgetId="homeimprove-tools" trackerHref="/dashboard" trackerLabel="View all tools" icon="hardware" iconClass="text-orange-500" />
          </SectionCard>
          <SectionCard title="Building Materials" icon="foundation" iconClass="text-slate-500">
            <FeedPreview widgetId="homeimprove-materials" trackerHref="/dashboard" trackerLabel="View building materials" icon="foundation" iconClass="text-slate-500" />
          </SectionCard>
          <SectionCard title="Plumbing" icon="water_pump" iconClass="text-blue-400">
            <FeedPreview widgetId="homeimprove-plumbing" trackerHref="/dashboard" trackerLabel="View plumbing supplies" icon="water_pump" iconClass="text-blue-400" />
          </SectionCard>
          <SectionCard title="Electrical" icon="electrical_services" iconClass="text-yellow-400">
            <FeedPreview widgetId="homeimprove-electrical" trackerHref="/dashboard" trackerLabel="View electrical supplies" icon="electrical_services" iconClass="text-yellow-400" />
          </SectionCard>
          <SectionCard title="DIY Deals" icon="sell" iconClass="text-amber-500">
            <FeedPreview widgetId="homeimprove-deals" trackerHref="/active-deals" trackerLabel="View all DIY deals" icon="sell" iconClass="text-amber-500" />
          </SectionCard>
        </div>
      </main>
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden flex items-center justify-around border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-[#101922] px-2 py-2">
        {[
          ...
          { href: '/home-improvement', label: 'DIY', icon: 'hardware' },
        ].map(n => (
