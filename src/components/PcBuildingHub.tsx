        ...
        preview.map((item, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
              <div className={`shrink-0 flex items-center justify-center size-8 rounded-lg ${iconClass.replace('text-', 'bg-').replace('-500', '-500/10')}`}>
...
// ─── Main component ───────────────────────────────────────────────────────────
export default function PcBuildingHub() {
  return (
    <div className="flex flex-col min-h-screen bg-[#f5f7f8] dark:bg-[#101922] font-[Space_Grotesk,system-ui,sans-serif] text-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-14 z-20 bg-[#f5f7f8]/80 dark:bg-[#101922]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center px-4 py-3 gap-3 max-w-3xl mx-auto w-full">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold">PC Building Hub</h1>
            <p className="text-xs text-slate-400">Track CPUs, RAM, GPUs and PC component deals</p>
          </div>
        </div>
      </header>
      ...
      <main className="pb-24 md:pb-6">
        <div className="max-w-3xl mx-auto w-full px-4 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <SectionCard title="CPU Availability" icon="memory_alt" iconClass="text-blue-500">
            <FeedPreview
              widgetId="cpu-availability"
              trackerHref="/cpu-availability-tracker"
              trackerLabel="Open CPU tracker"
              icon="memory_alt"
              iconClass="text-blue-500"
            />
          </SectionCard>

          <SectionCard title="RAM Availability" icon="memory" iconClass="text-purple-500">
            <FeedPreview
              widgetId="ram-availability"
              trackerHref="/ram-availability-tracker"
              trackerLabel="Open RAM tracker"
              icon="memory"
              iconClass="text-purple-500"
            />
          </SectionCard>

          <SectionCard title="GPU Availability" icon="videogame_asset" iconClass="text-green-500">
            <FeedPreview
              widgetId="gpu-availability"
              trackerHref="/gpu-availability-tracker"
              trackerLabel="Open GPU tracker"
              icon="videogame_asset"
              iconClass="text-green-500"
            />
          </SectionCard>

          <SectionCard title="PC Deals" icon="sell" iconClass="text-orange-500">
            <FeedPreview
              widgetId="pc-deals"
              trackerHref="/active-deals"
              trackerLabel="View all PC deals"
              icon="sell"
