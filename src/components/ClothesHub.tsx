...
export default function ClothesHub() {
  return (
    <div className="flex flex-col min-h-screen bg-[#f5f7f8] dark:bg-[#101922] font-[Space_Grotesk,system-ui,sans-serif] text-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-14 z-20 bg-[#f5f7f8]/80 dark:bg-[#101922]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center px-4 py-3 gap-3 max-w-3xl mx-auto w-full">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-400">Men's, women's & activewear deals</p>
          </div>
        </div>
      </header>
      <main className="pb-24 md:pb-6">
        <div className="max-w-3xl mx-auto w-full px-4 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <SectionCard title="New Arrivals" icon="new_releases" iconClass="text-slate-400">
            <FeedPreview widgetId="clothes-new" trackerHref="/dashboard" trackerLabel="View new arrivals" icon="new_releases" iconClass="text-slate-400" />
          </SectionCard>
          <SectionCard title="Clothing Deals" icon="sell" iconClass="text-amber-500">
            <FeedPreview widgetId="clothes-deals" trackerHref="/active-deals" trackerLabel="View all clothing deals" icon="sell" iconClass="text-amber-500" />
          </SectionCard>
          <SectionCard title="Men's Clothing" icon="man" iconClass="text-blue-500">
            <FeedPreview widgetId="clothes-mens" trackerHref="/dashboard" trackerLabel="View men's clothing" icon="man" iconClass="text-blue-500" />
          </SectionCard>
          <SectionCard title="Women's Clothing" icon="woman" iconClass="text-pink-500">
            <FeedPreview widgetId="clothes-womens" trackerHref="/dashboard" trackerLabel="View women's clothing" icon="woman" iconClass="text-pink-500" />
          </SectionCard>
          <SectionCard title="Activewear" icon="fitness_center" iconClass="text-green-500">
            <FeedPreview widgetId="clothes-activewear" trackerHref="/dashboard" trackerLabel="View activewear" icon="fitness_center" iconClass="text-green-500" />
          </SectionCard>
        </div>
      </main>
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden flex items-center justify-around border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-[#101922] px-2 py-2">
        {[
          ...
          { href: '/clothes', label: 'Clothes', icon: 'checkroom' },
        ].map(n => (
