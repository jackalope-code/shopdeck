        ...
        preview.map((item, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
              <div className={`shrink-0 flex items-center justify-center size-8 rounded-lg ${iconClass.replace('text-', 'bg-').replace('-500', '-500/10').replace('-400', '-400/10').replace('-600', '-600/10')}`}>
...
export default function GroceriesHub() {
  return (
    <div className="flex flex-col min-h-screen bg-[#f5f7f8] dark:bg-[#101922] font-[Space_Grotesk,system-ui,sans-serif] text-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-14 z-20 bg-[#f5f7f8]/80 dark:bg-[#101922]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center px-4 py-3 gap-3 max-w-3xl mx-auto w-full">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold">Groceries</h1>
            <p className="text-xs text-slate-400">Weekly deals, produce & pantry staples via Kroger</p>
          </div>
        </div>
      </header>
      ...
      <main className="pb-24 md:pb-6">
        <div className="max-w-3xl mx-auto w-full px-4 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <SectionCard title="Weekly Deals" icon="sell" iconClass="text-amber-500">
            <FeedPreview widgetId="grocery-deals" trackerHref="/active-deals" trackerLabel="View all grocery deals" icon="sell" iconClass="text-amber-500" />
          </SectionCard>
          <SectionCard title="Fresh Produce" icon="nutrition" iconClass="text-green-500">
            <FeedPreview widgetId="grocery-produce" trackerHref="/dashboard" trackerLabel="View all produce" icon="nutrition" iconClass="text-green-500" />
          </SectionCard>
          <SectionCard title="Pantry Staples" icon="kitchen" iconClass="text-yellow-600">
            <FeedPreview widgetId="grocery-staples" trackerHref="/dashboard" trackerLabel="View pantry staples" icon="kitchen" iconClass="text-yellow-600" />
          </SectionCard>
          <SectionCard title="Meat & Seafood" icon="set_meal" iconClass="text-red-500">
            <FeedPreview widgetId="grocery-meat-seafood" trackerHref="/dashboard" trackerLabel="View meat & seafood" icon="set_meal" iconClass="text-red-500" />
          </SectionCard>
        </div>
      </main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden flex items-center justify-around border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-[#101922] px-2 py-2">
        {[
          ...
          { href: '/groceries', label: 'Groceries', icon: 'shopping_cart' },
        ].map(n => (
