...
export default function ShoesHub() {
  return (
    <div className="flex flex-col min-h-screen bg-[#f5f7f8] dark:bg-[#101922] font-[Space_Grotesk,system-ui,sans-serif] text-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-14 z-20 bg-[#f5f7f8]/80 dark:bg-[#101922]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center px-4 py-3 gap-3 max-w-3xl mx-auto w-full">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-400">Athletic, casual & new release footwear deals</p>
          </div>
        </div>
      </header>
      <main className="pb-24 md:pb-6">
        <div className="max-w-3xl mx-auto w-full px-4 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <SectionCard title="New Releases" icon="new_releases" iconClass="text-green-500">
            <FeedPreview widgetId="shoes-new" trackerHref="/dashboard" trackerLabel="View new shoe releases" icon="new_releases" iconClass="text-green-500" />
          </SectionCard>
          <SectionCard title="Shoe Deals" icon="sell" iconClass="text-amber-500">
            <FeedPreview widgetId="shoes-deals" trackerHref="/active-deals" trackerLabel="View all shoe deals" icon="sell" iconClass="text-amber-500" />
          </SectionCard>
          <SectionCard title="Athletic & Running" icon="sprint" iconClass="text-blue-500">
            <FeedPreview widgetId="shoes-athletic" trackerHref="/dashboard" trackerLabel="View athletic shoes" icon="sprint" iconClass="text-blue-500" />
          </SectionCard>
          <SectionCard title="Casual & Lifestyle" icon="footprint" iconClass="text-slate-400">
            <FeedPreview widgetId="shoes-casual" trackerHref="/dashboard" trackerLabel="View casual shoes" icon="footprint" iconClass="text-slate-400" />
          </SectionCard>
        </div>
      </main>
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden flex items-center justify-around border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-[#101922] px-2 py-2">
        {[
          ...
          { href: '/shoes', label: 'Shoes', icon: 'footprint' },
        ].map(n => (
