...
function FeedPreview({ widgetId, trackerHref, trackerLabel, icon, iconClass }: {
  ...
  return (
    <div className="flex flex-col gap-2">
        ...
        preview.map((item, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
              <div className={`shrink-0 flex items-center justify-center size-8 rounded-lg ${iconClass.replace('text-', 'bg-').replace('-500', '-500/10').replace('-400', '-400/10')}`}>
              ...
              {price != null && price > 0 && (
                <p className="text-xs font-bold shrink-0">${price.toFixed(2)}</p>
              )}
            </div>
          );
        })
      ...
      <Link
        href={trackerHref}
        className="flex items-center justify-center gap-1 mt-1 text-xs font-semibold text-blue-500 hover:text-blue-600 transition-colors py-1"
      >
        {trackerLabel}
...
function SectionCard({ title, icon, iconClass, children }: {
  title: string;
  icon: string;
  iconClass: string;
    ...
    <div className="flex flex-col min-h-screen bg-[#f5f7f8] dark:bg-[#101922] font-[Space_Grotesk,system-ui,sans-serif] text-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-14 z-50 bg-[#f5f7f8]/80 dark:bg-[#101922]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center px-4 py-3 gap-3 max-w-3xl mx-auto w-full">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold">Garden Hub</h1>
            <p className="text-xs text-slate-400">Plants, seeds, tools & deals — zone-aware</p>
          </div>
        </div>
      </header>
      ...
      <main className="pb-24 md:pb-6">
        <div className="max-w-3xl mx-auto w-full px-4 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <SectionCard title="New Arrivals" icon="new_releases" iconClass="text-green-500">
            <FeedPreview
              widgetId="garden-new-arrivals"
              trackerHref="/dashboard"
              trackerLabel="View all garden arrivals"
              icon="new_releases"
              iconClass="text-green-500"
            />
          </SectionCard>

          <SectionCard title="Houseplants" icon="potted_plant" iconClass="text-emerald-500">
            <FeedPreview
              widgetId="garden-houseplants"
              trackerHref="/dashboard"
              trackerLabel="View all houseplants"
              icon="potted_plant"
              iconClass="text-emerald-500"
            />
          </SectionCard>

          <SectionCard title="Trees & Shrubs" icon="park" iconClass="text-teal-500">
            <FeedPreview
              widgetId="garden-trees-shrubs"
              trackerHref="/dashboard"
              trackerLabel="View all trees & shrubs"
              icon="park"
              iconClass="text-teal-500"
            />
          </SectionCard>

          <SectionCard title="Perennials" icon="grass" iconClass="text-green-400">
            <FeedPreview
              widgetId="garden-perennials"
              trackerHref="/dashboard"
              trackerLabel="View all perennials"
              icon="grass"
              iconClass="text-green-400"
            />
          </SectionCard>

          <SectionCard title="Seeds" icon="compost" iconClass="text-lime-500">
            <FeedPreview
              widgetId="garden-seeds"
              trackerHref="/dashboard"
              trackerLabel="View all seeds"
              icon="compost"
              iconClass="text-lime-500"
            />
          </SectionCard>

          <SectionCard title="Garden Tools" icon="construction" iconClass="text-orange-400">
            <FeedPreview
              widgetId="garden-tools"
              trackerHref="/dashboard"
              trackerLabel="View all garden tools"
              icon="construction"
              iconClass="text-orange-400"
            />
          </SectionCard>

          <SectionCard title="Garden Deals" icon="sell" iconClass="text-amber-500">
            <FeedPreview
              widgetId="garden-deals"
              trackerHref="/active-deals"
              trackerLabel="View all garden deals"
              icon="sell"
