...
export default function CraftsHub() {
  return (
    <div className="flex flex-col min-h-screen bg-[#f5f7f8] dark:bg-[#101922] font-[Space_Grotesk,system-ui,sans-serif] text-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-14 z-20 bg-[#f5f7f8]/80 dark:bg-[#101922]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center px-4 py-3 gap-3 max-w-3xl mx-auto w-full">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold">Crafts</h1>
            <p className="text-xs text-slate-400">Pottery, weaving, fiber arts & craft supply deals</p>
          </div>
        </div>
      </header>
      <main className="pb-24 md:pb-6">
        <div className="max-w-3xl mx-auto w-full px-4 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <SectionCard title="Craft Deals" icon="sell" iconClass="text-amber-500">
            <FeedPreview widgetId="crafts-deals" trackerHref="/active-deals" trackerLabel="View all craft deals" icon="sell" iconClass="text-amber-500" />
          </SectionCard>
          <SectionCard title="Pottery" icon="blur_circular" iconClass="text-orange-400">
            <FeedPreview widgetId="crafts-pottery" trackerHref="/dashboard" trackerLabel="View pottery supplies" icon="blur_circular" iconClass="text-orange-400" />
          </SectionCard>
          <SectionCard title="Weaving & Fiber Arts" icon="grid_on" iconClass="text-teal-500">
            <FeedPreview widgetId="crafts-weaving" trackerHref="/dashboard" trackerLabel="View weaving supplies" icon="grid_on" iconClass="text-teal-500" />
          </SectionCard>
        </div>
      </main>
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden flex items-center justify-around border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-[#101922] px-2 py-2">
        {[
          ...
          { href: '/crafts', label: 'Crafts', icon: 'draw' },
        ].map(n => (
