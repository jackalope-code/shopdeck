...
export default function AutomotiveHub() {
  return (
    <div className="flex flex-col min-h-screen bg-[#f5f7f8] dark:bg-[#101922] font-[Space_Grotesk,system-ui,sans-serif] text-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-14 z-50 bg-[#f5f7f8]/80 dark:bg-[#101922]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center px-4 py-3 gap-3 max-w-3xl mx-auto w-full">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-400">Parts, tools & accessories deals</p>
          </div>
        </div>
      </header>
      <main className="pb-24 md:pb-6">
        <div className="max-w-3xl mx-auto w-full px-4 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <SectionCard title="Auto Parts" icon="directions_car" iconClass="text-blue-500">
            <FeedPreview widgetId="auto-parts" trackerHref="/dashboard" trackerLabel="View all auto parts" icon="directions_car" iconClass="text-blue-500" />
          </SectionCard>
          <SectionCard title="Tools & Equipment" icon="build" iconClass="text-orange-400">
            <FeedPreview widgetId="auto-tools" trackerHref="/dashboard" trackerLabel="View all tools" icon="build" iconClass="text-orange-400" />
          </SectionCard>
          <SectionCard title="Car Accessories" icon="auto_awesome" iconClass="text-teal-400">
            <FeedPreview widgetId="auto-accessories" trackerHref="/dashboard" trackerLabel="View all accessories" icon="auto_awesome" iconClass="text-teal-400" />
          </SectionCard>
          <SectionCard title="Auto Deals" icon="sell" iconClass="text-amber-500">
            <FeedPreview widgetId="auto-deals" trackerHref="/active-deals" trackerLabel="View all auto deals" icon="sell" iconClass="text-amber-500" />
          </SectionCard>
        </div>
      </main>
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden flex items-center justify-around border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-[#101922] px-2 py-2">
        {[
          ...
          { href: '/automotive', label: 'Auto', icon: 'directions_car' },
        ].map(n => (
