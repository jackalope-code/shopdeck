    ...
    <div className="flex flex-col min-h-screen bg-[#f5f7f8] dark:bg-[#101922] font-[Space_Grotesk,system-ui,sans-serif] text-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-14 z-50 bg-[#f5f7f8]/80 dark:bg-[#101922]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center px-4 py-3 gap-3 max-w-3xl mx-auto w-full">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold">3D Printing</h1>
            <p className="text-xs text-slate-400">Printers, filament, resin & accessories</p>
          </div>
        </div>
      </header>
      <main className="pb-24 md:pb-6">
        <div className="max-w-3xl mx-auto w-full px-4 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <SectionCard title="3D Printers" icon="print" iconClass="text-blue-500">
            <FeedPreview widgetId="3dp-printers" trackerHref="/dashboard" trackerLabel="View all 3D printers" icon="print" iconClass="text-blue-500" />
          </SectionCard>
          <SectionCard title="Filament" icon="cable" iconClass="text-yellow-400">
            <FeedPreview widgetId="3dp-filament" trackerHref="/dashboard" trackerLabel="View all filament" icon="cable" iconClass="text-yellow-400" />
          </SectionCard>
          <SectionCard title="Resins" icon="science" iconClass="text-purple-500">
            <FeedPreview widgetId="3dp-resins" trackerHref="/dashboard" trackerLabel="View all resins" icon="science" iconClass="text-purple-500" />
          </SectionCard>
          <SectionCard title="Accessories & Upgrades" icon="build" iconClass="text-slate-400">
            <FeedPreview widgetId="3dp-accessories" trackerHref="/dashboard" trackerLabel="View accessories & upgrades" icon="build" iconClass="text-slate-400" />
          </SectionCard>
          <SectionCard title="3D Printing Deals" icon="sell" iconClass="text-amber-500">
            <FeedPreview widgetId="3dp-deals" trackerHref="/active-deals" trackerLabel="View all 3D printing deals" icon="sell" iconClass="text-amber-500" />
          </SectionCard>
        </div>
      </main>
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden flex items-center justify-around border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-[#101922] px-2 py-2">
        {[
          ...
          { href: '/3d-printing', label: '3D Print', icon: 'print' },
        ].map(n => (
